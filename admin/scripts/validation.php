...
function signupValidation(string $fname, string $email, string $username, $password, $confirm)
{
    ...
    if (preg_match("/^\w{8,}$/", $password) == 0) {
        die(json_encode(["STATUS_CODE" => "0056", "STATUS_TITLE" => "Invalid password!", "STATUS_TEXT" => "Password doesn't meet our criteria."]));
    }
    return true;
}