...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $result = $db->executeSql("SELECT * FROM projects WHERE project_unique_identifier = ?", [$data], true)[0];
    $path = "../upload/uploads/" . $result["file_path"];
    $path = realpath($path);
    if (unlink($path)) {
        $db->executeSql("DELETE FROM projects WHERE project_unique_identifier = ?", [$data]);
